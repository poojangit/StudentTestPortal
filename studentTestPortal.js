...
function takeTest() {
    ...
    for (let student of studentDetails) {
        student.test_score = subjects.map(subject => ({
            sub_name: subject,
    ...
    testTaken = true
    console.log("Test scores have been generated for all students. along with total marks and percentage");
    mainMenu()
}
...
function viewStudentsResult() {
    ...
    if (student) {
        if (!student.totalMarks || !student.percentage) {
            console.log(`Results for ${student.Name} are not generated. Please take the test.`);
        } else {
